...
void mbr_parse(char letter)
{
	...
	/* } */
        ...
        /* no MBR so mock up data that would have been extracted */
        blk_op.blkdev->lba_first[k] = 0x00031000;
        blk_op.blkdev->lba_count[k] = 65535;
        kprintf("hd%c%d ", letter, k+1);
        k++;

	/* /\* add each entry to blkops struct *\/ */
	/* /\*  This adds partitions as it finds good ones? *\/ */
